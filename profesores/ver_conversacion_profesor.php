...
if (isset($_SESSION['rut'])) {
    ...
    // Actualizar el campo leído en la tabla mensajes para marcar los mensajes como leídos al cargar la página
    $actualizar_leidos_query = "UPDATE mensajes SET leido = 1 WHERE idConversacion = $idConversacion AND idReceptor = '$rut_profesor'";
    $resultado_actualizar_leidos = mysqli_query($conexion, $actualizar_leidos_query);

    if (!$resultado_actualizar_leidos) {
        die("Error al marcar mensajes como leídos: " . mysqli_error($conexion));
    }
    ...
    if ($profesor_result) {
        ...
        die("Error al obtener el nombre del profesor: " . mysqli_error($conexion));
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        header("Location: {$_SERVER['PHP_SELF']}?idConversacion=$idConversacion&idCurso=$idCurso&idAsignatura=$idAsignatura&idEmisor=$idEmisor");
        exit();
    }
